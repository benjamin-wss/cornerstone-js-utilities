 ...
 * @returns {string}
 */
function getEnvVariableAsString({
  fieldName,
  defaultValue,
  allowedValues = [],
}) {
  ...
  if (Yup.array().required().min(1).isValidSync(allowedValues)) {
    const isFieldValueValid = Yup.string()
      .required()
      .oneOf(allowedValues)
      .isValidSync(value);
 ...
 */
function getEnvironmentVariableAsStringArray({
  fieldName,
  defaultValue,
  delimiter = ",",
}) {
  ...
  if (splitValue.length < 1) {
    throw new Error(`
      Unable to derive string array for field name ${fieldName}.
    `);
  }

  return splitValue.map((x) => x.trim());
}
